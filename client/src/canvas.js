...
export default function Canvas() {
  ...
  const localUndoStackRef = useRef([]);
  ...
  useEffect(() => {
    ...
    const stopDrawing = () => {
      if (!drawing || !canvas || !ctx) return;
       if (currentSegmentsRef.current.length === 0) {
          ...
          if(socket.connected) socket.emit("stopDrawing", { username: usernameRef.current });
          return;
      }
      ...
      if (socket.connected) {
        ...
        if (strokeId && currentSegmentsRef.current.length > 0) {
            const currentStroke = {
                id: strokeId,
                segments: [...currentSegmentsRef.current]
            };
            socket.emit("addStroke", currentStroke);
            drawHistoryRef.current.push({ ...currentStroke, userId: socket.id, undone: false });
            if (drawHistoryRef.current.length > MAX_DRAW_HISTORY_CLIENT) {
                drawHistoryRef.current.shift(); //Remove oldest element
            }
            localUndoStackRef.current.push(strokeId);
    ...
    socket.on("strokeUndoStateChanged", ({ strokeId, undone }) => {
        ...
        const strokeIndex = history.findIndex(s => s && s.id === strokeId);
        if (strokeIndex !== -1) {
            //Directly modify the object in the ref array
            history[strokeIndex].undone = undone;

            //Adjust local stacks ONLY if it was our action confirmed by server
            if (history[strokeIndex].userId === socket.id) {
                if (undone) { //Confirmed Undo
                    const indexInUndo = localUndoStackRef.current.indexOf(strokeId);
                    if (indexInUndo > -1) localUndoStackRef.current.splice(indexInUndo, 1);
                    if (!localRedoStackRef.current.includes(strokeId)) localRedoStackRef.current.push(strokeId);
                } else { // Confirmed Redo
                    const indexInRedo = localRedoStackRef.current.indexOf(strokeId);
                    if (indexInRedo > -1) localRedoStackRef.current.splice(indexInRedo, 1);
                    if (!localUndoStackRef.current.includes(strokeId)) localUndoStackRef.current.push(strokeId);
                }
            }
            redrawCanvasFromHistory(); //Trigger redraw
        } else {
        ...
        if (socket.connected) {
          if (isUndo) {
              if (localUndoStackRef.current.length > 0) {
                  //Send the ID of the last item on the stack
                  const strokeIdToUndo = localUndoStackRef.current[localUndoStackRef.current.length - 1];
                  socket.emit("attemptUndo", strokeIdToUndo);
                  // Don't pop or push here, wait for server confirmation
              }
          } else if (isRedo) {
              if (localRedoStackRef.current.length > 0) {
                   //Send the ID of the last item on the stack
                  const strokeIdToRedo = localRedoStackRef.current[localRedoStackRef.current.length - 1];
                  socket.emit("attemptRedo", strokeIdToRedo);
